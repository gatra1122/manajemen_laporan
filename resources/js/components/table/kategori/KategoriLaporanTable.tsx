...
import { KategoriLaporanType } from '../../../types';
...
import KategoriModal from './KategoriLaporanModal';
...
const KategoriLaporanTable = () => {
    const [search, setSearch] = useState('');
    const [debouncedSearch] = useDebounce(search, 500);
    const [selectedData, setSelectedData] = useState<KategoriLaporanType | null>(null);
    ...
    const { data, error, isFetching } = useQuery({
        queryKey: ['kategori_laporan', pagination.pageIndex, debouncedSearch],
        queryFn: () => {
            return axiosClient.get('/kategori_laporan', {
                params: {
    ...
    const columnHelper = createColumnHelper<KategoriLaporanType>()
    const columns = [
        {
            id: 'row-number',
            header: '#',
...
export default KategoriLaporanTable;